...
public class Profile : MonoBehaviour
{
	...
	void SelectAvatar (int AvatarIndex)
	{
        ...
        foreach (var item in Shop.Instance.ShopItemsList)
        {
			if (item.Image == CurrentAvatar.sprite)
            {
				PlayerPrefs.SetInt("selectedPlayerId", item.id); // get selected player id
				PlayerPrefs.SetString("playerName", item.name);
				textList[0].text = item.name;
				textList[1].text = "Speed : " + PlayerPrefs.GetFloat(item.name + "speedX", 0).ToString();
				textList[2].text = "Fuel Second : " + PlayerPrefs.GetFloat(item.name + "maxValue", 0).ToString();
			}
        }
	}
}
